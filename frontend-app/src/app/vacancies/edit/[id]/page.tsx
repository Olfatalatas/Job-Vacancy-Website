...
// --- KOMPONEN 1: FORMULIR (Hanya dirender saat data sudah ada) ---
function EditForm({ initialData, id }: { initialData: Vacancy; id: string }) {
  ...
  const [formData, setFormData] = useState({
    ...
    salary_min: Number(initialData.salary_min) || 0,
    salary_max: Number(initialData.salary_max) || 0,
    show_salary: Boolean(initialData.show_salary),
    min_experience: initialData.min_experience,
  });

  const mutation = useMutation({
    mutationFn: async (updatedData: typeof formData) => {
      return await api.put(`/vacancies/${id}`, updatedData);
    },
    onSuccess: () => {
...
// --- KOMPONEN 2: HALAMAN UTAMA (Data Fetching Only) ---
export default function EditVacancyPage() {
  const { id } = useParams();
  const router = useRouter();

  // 1. Fetch Data Lama
  const { data: oldData, isLoading, isError } = useQuery({
    queryKey: ['vacancy', id],
    queryFn: async () => {
  ...
  if (isError || !oldData) return <div className="min-h-screen flex items-center justify-center">Gagal memuat data.</div>;
      ...
      <main className="py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-gray-600">Perbarui informasi lowongan pekerjaan ini.</p>
          </div>

          <EditForm initialData={oldData} id={id as string} />
        </div>
      </main>
    </div>
  );
